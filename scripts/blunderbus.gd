...
@export var bullet_icon = preload("res://sprites/B_Bullet_icon.png")
@export var empty_bullet_icon = preload("res://sprites/Empty_B_Bullet_icon2.png")
...
func shoot():
	if current_ammo > 0:
		...
		new_bullet2.global_position = %ShotingPoint.global_position
		new_bullet2.global_rotation = %ShotingPoint.global_rotation + spread
		get_tree().root.get_node("Game").add_child(new_bullet2)		
		new_bullet3.global_position = %ShotingPoint.global_position
		new_bullet3.global_rotation = %ShotingPoint.global_rotation - spread
		get_tree().root.get_node("Game").add_child(new_bullet3)
		update_ammo()
		update_ammo_display()
	else: 
